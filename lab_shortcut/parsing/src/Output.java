...
public class Output {
    ...
    public static void ret(String register){
        var type = identifier.getType(register);
        println("ret %s %s", type, register);
    }
    ...
    public static void call(String func, List<String> list, String reg){
        ...
        for (int i = 0; i < list.size(); i++){
            var params = list.get(i);
            var type = identifier.getType(params);
            if (i == list.size() - 1){
                System.out.printf("%s %s", type, params);
            }
            else{
                System.out.printf("%s %s,", type, params);
            }
    ...
    public static void declare(String func){
        String funcType = identifier.getFuncType(func);
        System.out.printf("declare %s @%s(", funcType, func);
        var list = identifier.getFuncParams(func);
        for (int i = 0; i < list.size(); i++){
            var params = list.get(i);
            if (i == list.size() - 1){
    ...
    public static void getelementptr(String reg1, String reg2, int pos1, String pos2){
        var type = identifier.getType(reg2);
        println("%s = getelementptr %s, %s %s, i32 %d, i32 %s", reg1, type.replace("*", ""), type, reg2, pos1, pos2);
    }
